...
OTU5_tab$Longitude = OTU5_tab$Longitude + runif(n=length(OTU5_tab$Longitude), min=0, max=0.0001)
OTU5_tab$Latitude = OTU5_tab$Latitude + runif(n=length(OTU5_tab$Latitude), min=0, max=0.0001)
...
####################################################################################
xmin_plot = extent(my.sp)[1] 
xmax_plot = extent(my.sp)[2] 
ymin_plot = extent(my.sp)[3] 
ymax_plot = extent(my.sp)[4] 
...
ggplot() +
  geom_point(data = vgOK, aes(x = dist/1000, y = gamma)) +
...
vg <- ggplot(data = vgOK, aes(x = distance_km, y = gamma)) +
  geom_point() +
  xlab("Distance (km)") +
  ylab("semivariance") +
  theme_bw()
